...
if (!is_admin()) {
        ...
        if (preg_match('/author=([0-9]*)/i', $_SERVER['QUERY_STRING'])) { 
			status_header(404);
			header('Location: http://www.camaravinhedo.sp.gov.br/');
			die();
		}
...
function wpse72395_register_tinymce_plugin($plugin_array) {
    $plugin_array['wpse72395_button'] = get_template_directory_uri() . '/library/accordion-btn.js';
    return $plugin_array;
}
...
function remove_tinymce_buttons( $buttons ) {
      ...
      if ( ( $key = array_search( 'wp_adv', $buttons ) ) !== false )
		unset( $buttons[$key] );
      if ( ( $key = array_search( 'link', $buttons ) ) !== false )
		unset( $buttons[$key] );
      if ( ( $key = array_search( 'unlink', $buttons ) ) !== false )
		unset( $buttons[$key] );
      return $buttons;
 }
...
function add_tinymce_buttons( $buttons ) {
	  ...
	  array_push( $buttons, 'alignjustify','link', 'unlink', 'wp_adv' );
      return $buttons;
}
...
function remove_tinymce_buttons_2( $buttons ) {
      if ( ( $key = array_search( 'strikethrough', $buttons ) ) !== false )
		unset( $buttons[$key] );
      return $buttons;
 }
add_filter( 'mce_buttons_2', 'remove_tinymce_buttons_2' );

// Styleselect (including links)
...
function my_column_width() {
	...
	.wpsm-collaps-C-review-notice, .mce-toolbar-grp #mceu_1 { display: none !important; }
	.mce-toolbar-grp #mceu_0 { background-color: white; border: 1px solid #ddd; -webkit-box-shadow: inset 0 1px 1px -1px rgba(0,0,0,.2); box-shadow: inset 0 1px 1px -1px rgba(0,0,0,.2); border-radius: 0; }
	.mce-toolbar-grp #mceu_0 .mce-txt { padding-right: 27px }
...
function cinza( $atts, $content = null ) {
	return '<div class="panel-body">' . do_shortcode($content) . '</div>';
}
add_shortcode( 'blococinza', 'cinza' );
